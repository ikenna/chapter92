...
public class CategoriesResponseLinks {
  ...
  * @throws IOException if the JSON Element is invalid with respect to CategoriesResponseLinks
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!CategoriesResponseLinks.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in CategoriesResponseLinks is not found in the empty JSON string", CategoriesResponseLinks.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!CategoriesResponseLinks.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `CategoriesResponseLinks` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }

      // check to make sure all required properties/fields are present in the JSON string
      for (String requiredField : CategoriesResponseLinks.openapiRequiredFields) {
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!CategoriesResponseLinks.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'CategoriesResponseLinks' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<CategoriesResponseLinks> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(CategoriesResponseLinks.class));

       return (TypeAdapter<T>) new TypeAdapter<CategoriesResponseLinks>() {
           @Override
           public void write(JsonWriter out, CategoriesResponseLinks value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public CategoriesResponseLinks read(JsonReader in) throws IOException {
 ...
 /**
  * Create an instance of CategoriesResponseLinks given an JSON string
  *
  * @param jsonString JSON string
  * @return An instance of CategoriesResponseLinks
  * @throws IOException if the JSON string is invalid with respect to CategoriesResponseLinks
  */
  public static CategoriesResponseLinks fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, CategoriesResponseLinks.class);
  }

 /**
  * Convert an instance of CategoriesResponseLinks to an JSON string
  *
  * @return JSON string
  */
